      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-bold text-[#2C2C2C] mb-4 font-serif">
            English Language Requirements – USA & Ireland
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {countryData.map((country, idx) => (
            <div
              key={idx}
              className={`${country.bgColor} rounded-xl p-5 shadow-2xl border ${country.borderColor}`}
            >
              {/* Header */}
              <div className="flex items-center mb-6">
                <div className="p-3 rounded-full mr-4" style={{ backgroundColor: country.accentColor }}>
              ...
              <div className="block md:hidden space-y-4">
                {country.exams.map((exam, i) => (
                  <div key={i} className="border border-gray-300 rounded-lg p-4 bg-white shadow text-sm">
                    <p className="font-semibold text-[#2C2C2C]">Test: <span className="font-normal">{exam.test}</span></p>
                    <p className="font-semibold text-[#2C2C2C]">UG: <span className="font-normal">{exam.ug}</span></p>
                    <p className="font-semibold text-[#2C2C2C]">PG: <span className="font-normal">{exam.pg}</span></p>
                    <p className="font-semibold text-[#2C2C2C]">Max Score: <span className="font-normal">{exam.max}</span></p>
                  </div>
                ))}
              </div>
              ...
              <p className="mt-4 text-sm text-gray-700">
                <Info className="inline-block w-4 h-4 mr-1 mb-1" style={{ color: country.accentColor }} />
